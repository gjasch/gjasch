...
function proceedToNextLevel() {
    // currentLevel has already been incremented before this function is called
    // (it happens in the freezeFrame -> levelComplete transition)
    
    scoreAtLevelStart = score; // <<< ADD THIS LINE: "Bank" the score from the completed level

    initializeEnemies();    
    initializeBarriers();   
    resetPlayerPosition();  // This resets player.isInvincible, player.hasAutoFire etc.
    bullets = [];           
    enemyBullets = [];      
    particles = []; 
    fallingObjects = []; 
...
function startGame(isContinuing = false) {
  if (!isContinuing) {
    score = 0; 
    currentLevel = 1; 
    scoreAtLevelStart = 0; // <<< ADD THIS LINE
    // powerupMessage = ""; // These are no longer global
    // activePowerupNameForMessage = "";
    // activePowerupTimerDisplay = 0;
  } else { // This is a "Continue" from Game Over
    score = scoreAtLevelStart; // Reset score to what it was at the start of the failed level
  }
...
document.addEventListener('keyup', function(event) {
  if (gameState !== "playing") return; // Only handle keyup for playing state related actions
  if (event.key === 'ArrowLeft') {
...
canvas.addEventListener('touchstart', function(e) {
    ...
    for (let i = 0; i < e.changedTouches.length; i++) {
        ...
        if (gameState === "levelComplete") {
            if (isInside(touchPos, readyButtonLevelComplete)) { 
                proceedToNextLevel();
                actionTaken = true;
                break; 
            }
        } else if (gameState === "playing" && onScreenControlsEnabled) {
        ...
        // No specific touch actions for "paused", "title", "settings", "gameOver" on osFireButton
        // as those are handled by their specific large menu buttons via the main 'click' listener.
    }
    if (actionTaken) {
        e.preventDefault();
    }
